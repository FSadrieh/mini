...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        data_config = {
            ...
            "firstn_datasets": cfg.data.get("firstn_datasets", None),
            "seed": cfg.data.get("seed", self.seed),
            "data_location": cfg.data.get("data_location", "data"),
            "preprocessing_workers": cfg.data.get("preprocessing_workers", 1),
        }
    ...
    def _loss_step(self, batch: dict[str, torch.Tensor], mode = "train") -> torch.Tensor:
        ...
        # Compute accuracy
        predictions = torch.argmax(outputs.logits[:, :-1, :].contiguous(), axis=-1).view(-1)
        ...
        custom_loss = self.calculate_loss(outputs.logits, batch["labels"], batch["sample_count"])

        self.log_dict(
            {
                f"{mode}/accuracy": accuracy,
                f"{mode}/mean_loss": custom_loss["mean"],
                f"{mode}/sum_loss": custom_loss["sum"],
                f"{mode}/distance_loss": custom_loss["distance"],
                f"{mode}/var_loss": custom_loss["variance"],
            },
            on_step=mode == "train",
            on_epoch=mode != "train",
