...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def _setup_model(
        ...
        # base_model.eval()
        # print("dtype:", next(base_model.parameters()).dtype)
        # print("device:", next(base_model.parameters()).device)
        # tokens = self.convert_prompt_to_tokens({"system": None, "user": "Tell me a joke."})
        # out, _ = generation.generate(
        #     model=base_model,
        #     prompt=torch.tensor([tokens], device=self._device),
        #     max_generated_tokens=50, temperature=0.6, top_k=300,
        #     pad_id=self._tokenizer.pad_id,
        #     stop_tokens=self._tokenizer.stop_tokens
        # )
        # print("RAW TOKENS:", out[0][:20])
        # print("DECODED:", self._tokenizer.decode(out[0]))
