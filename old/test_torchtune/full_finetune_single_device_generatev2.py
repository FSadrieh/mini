...
### Custom imports for this recipe ###
...
class FullFinetuneRecipeSingleDevice(FTRecipeInterface):
    ...
    def generate(self):
        cfg = {
            "prompt": {"system": "You are a helpful and creative AI assistant.", "user": "What is the capital of France?"},
            "max_new_tokens": 10,
            "temperature": 0.6,
            "top_k": 300,
        }
        self.model_transform = self._tokenizer
        ...
        batch = {}
        if isinstance(self._model, DeepFusionModel) and is_multimodal_input:
        ...
        elif isinstance(self._model, EarlyFusionModel) and is_multimodal_input:
        ...
        generated_tokens = []
        t0 = time.perf_counter()
        logits = self._model(prompt, **batch)[:, -1]
        ...
        for i in range(cfg["max_new_tokens"]):
            ...
            logits = self._model(token, **batch)[:, -1]
            token = sample(logits, temperature=cfg['temperature'], top_k=cfg['top_k'])
            generated_tokens.append(token.item())
            seq_len += 1
